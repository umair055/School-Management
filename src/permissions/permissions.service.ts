...
export class PermissionsService {
  ...
  mergePermissionObjects(rolePermissions) {
    try {
      ...
      allPermissionTypes.forEach((permissionType) => {
        ...
        CRUD.forEach((key) => {
          permissionsObject[permissionType][key] = rolePermissions[permissionType]?.[key];
        });
      });
   ...
   * @returns
   */
  mergePermissions(userRoles: UserRoleEntity[]) {
    try {
      const rolePermissions = this.mergeRolePermissions(userRoles);
      const mergePermissionObject = this.mergePermissionObjects(rolePermissions);
      delete mergePermissionObject['id'];
      return mergePermissionObject;
    } catch (error) {
      Logger.error(
        `Error in mergePermissions of AuthService where userRoles: ${JSON.stringify(userRoles)}`,
      );
      throw error;
    }
  }
